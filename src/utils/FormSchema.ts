...
export const emailSchema = yup.object().shape({ email: emailValidation }).required();
export const passwordSchema = yup.object().shape({ password: passwordValidation }).required();
...
export const updateUserSchema = yup
  ...
  .required();
export const updateAdminSchema = yup.object().shape({ name: stringValidation }).required();
...
export const clientSchema = yup.object().shape({
  ...
  investigation_file_number: yup
    .string()
    .matches(/^[a-zA-Z0-9\-\/]*$/, 'Solo se permiten letras, números, guiones (-) y diagonales (/)')
    .optional(),
  ...
  payment_frequency: yup.string().when('status', {
    ...
    then: (schema) =>
      schema
        .oneOf(
          ['Mensual', 'Bimestral', 'Trimestral', 'Semestral', 'Contado'],
  ...
  bracelet_type: yup.string().when('status', {
    ...
    then: (schema) =>
      schema
        .oneOf(['B1', 'G2', 'Otro'], 'El tipo de brazalete debe ser: B1, G2 o Otro')
  ...
  transfer_reason: yup.string().when('status', {
    is: 'Traspaso',
    then: (schema) => schema.required('El motivo de traspaso es obligatorio cuando el estado es "Traspaso"'),
    otherwise: (schema) => schema.optional(),
  }),
});
export const carrierSchema = yup.object().shape({
  ...
  relationship_id: integerValidation.oneOf([1, 2], 'Seleccione un parentesco válido'),
});
export const contractSchema = yup.object().shape({ contract: fileValidation });
export const reportSchema = yup.object().shape({ installation_report: fileValidation });
